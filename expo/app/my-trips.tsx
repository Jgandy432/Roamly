...
function TripCard({ trip, onPress }: { trip: Trip; onPress: () => void }) {
  ...
  const submittedCount = trip.members.filter((member) => member.preferencesSubmitted).length;
      ...
      <TouchableOpacity style={styles.tripCard} onPress={onPress} onPressIn={handlePressIn} onPressOut={handlePressOut} activeOpacity={1}>
        <View style={styles.tripCardTop}>
          <View style={styles.tripCardMeta}>
            ...
            <Text style={styles.tripDest}>{trip.destination} · {trip.members.length} collaborators</Text>
          </View>
          <View style={styles.statusPill}><Text style={styles.statusPillText}>{trip.status}</Text></View>
        </View>
        <View style={styles.divider} />
        <View style={styles.tripCardBottom}>
          <Text style={styles.memberCountText}>{submittedCount}/{totalCount} preferences submitted</Text>
        </View>
...
export default function MyTripsScreen() {
  ...
  const { trips, currentUser, setActiveTrip, setActiveTripPlan } = useTrips();
        ...
        <Animated.View style={[styles.flex, { opacity: fadeAnim }]}> 
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.joinCard}>
              <Text style={styles.joinCardLabel}>Trips are account-based now</Text>
              <Text style={styles.joinCardBody}>Owners invite collaborators by email. Accepted invites add real user accounts directly to trip membership.</Text>
            </View>
            ...
            <View style={styles.tripList}>
              {displayedTrips.length === 0 ? (
                <View style={styles.emptyState}>
                  ...
                  <Text style={styles.emptyBody}>{activeTab === 'active' ? 'Create a trip and invite real collaborators by email.' : 'Your completed account-backed trips will live here.'}</Text>
                  {activeTab === 'active' && (
                ...
                displayedTrips.map((trip) => <TripCard key={trip.id} trip={trip} onPress={() => handleOpenTrip(trip)} />)
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 28, fontWeight: '800' as const, color: Colors.text },
  ...
  joinCardLabel: { fontSize: 13, fontWeight: '700' as const, color: Colors.textSecondary, marginBottom: 8 },
  joinCardBody: { fontSize: 13, color: Colors.textMuted, lineHeight: 20 },
  ...
  statusPill: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 50, backgroundColor: Colors.bgInput },
  statusPillText: { fontSize: 10, fontWeight: '700' as const, color: Colors.textSecondary, textTransform: 'capitalize' as const },
  divider: { height: 1, backgroundColor: Colors.border, marginBottom: 12 },
  tripCardBottom: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  memberCountText: { fontSize: 12, color: Colors.textMuted, fontWeight: '500' as const },
  progressBarTrack: { height: 4, backgroundColor: Colors.border, borderRadius: 50, marginBottom: 8, overflow: 'hidden' },
  progressBarFill: { height: '100%', borderRadius: 50, backgroundColor: Colors.orange },
